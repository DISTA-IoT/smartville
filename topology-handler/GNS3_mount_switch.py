...
def mountSwitch(PROJECT_NAME,SWITCH_NAME,SWITCH_LABEL,ip,gateway):
    ...
    if(switch_template_id is not None):  
        ...
        print((f"{SWITCH_LABEL}: deleting old switch template"))
    ...
    start_node(server,project,openvswitch_id)
    print(f"{SWITCH_LABEL}: started")
    
def mountSwitchDHCP(PROJECT_NAME,SWITCH_NAME,SWITCH_LABEL, interfaces: List[str]):
    server = Server(*read_local_gns3_config())
    project = get_project_by_name(server, PROJECT_NAME)
    ...
    openvswitch=create_node(server, project, 0, -200, switch_template_id,SWITCH_LABEL)
    print(f"{SWITCH_LABEL}: created")
    openvswitch_id = openvswitch['node_id']

    for interface in interfaces:
        set_dhcp_node_network_interfaces(server, project, openvswitch_id, interface)
        print(f"{SWITCH_LABEL}: DHCP on ",interface)
